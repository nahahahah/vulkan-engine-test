...
ShaderModule::ShaderModule(VkShaderModuleCreateInfo const& createInfo, Device const& device) : _device(device) {
    ...
    if (result != VK_SUCCESS) {
        std::string error = "Unable to create the vertex shader module: (status: " + std::to_string(result) + ")";
        throw std::runtime_error(error);
    }
    std::cerr << "Vertex shader module created successfully: <VkShaderModule " << _handle << ">" << std::endl;
}
